...
/** Leading display flag for a ticket post (share text, cards, OG). */
export function resolveTicketPostFlag(
  ...
  if (matchFlags?.flag1 && matchFlags?.flag2) {
    return `${matchFlags.flag1}${matchFlags.flag2}`;
  }
  if (matchFlags?.flag1) return matchFlags.flag1;
  if (!isPlaceholderFlag(post.flag)) return post.flag!.trim();
  const p = post.payload as Record<string, unknown> | null | undefined;
